...
public class SnSLoginController {
   ...
   public ModelAndView kakaoLogin(HttpServletRequest request) {
      ...
      // 2. 토큰받기(인가코드 필요)
      String reqURL = "https://kauth.kakao.com/oauth/token";//토큰 요청 url
       try {
            ...
            // 헤더 요청
            conn.setRequestProperty("Content-type", "application/x-www-form-urlencoded;charset=utf-8");//카카오에서 요구한 포멧 중 하나
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
            	...
            	//가져온 정보를 바탕으로 DB넣거나 session으로 저장
            	Gson gson = new Gson();
   ...
   public ModelAndView daumAddrOk(AddrVO addrvo, HttpSession session) {
	   ...
	   // 로그인 성공한 상태에서 session 안에 있는 사용자 덩보 둥 m_id 나 사용자 테이블의 pk를 호출한다.
