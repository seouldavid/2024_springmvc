...
<body>
	...
	<table>
					...
					<c:forEach var="k" items="${cart_list }">
						<tr>
							...
							<td>시중가 : <fmt:formatNumber value="${k.p_price}"
									pattern="#,##0" /> 원 <br> <font color="red">(할인가 :
									<fmt:formatNumber value="${k.p_saleprice}" pattern="#,##0" />
									원)
							</font>
							</td>
							<td>
							...
							<td><fmt:formatNumber value="${k.p_price * k.p_su }"
									pattern="#,##0" /> 원</td>
		...
		<tfoot>
			<tr>
				<td colspan="6" style="text-align: right; padding: 10px 50px;">총
					결재액 : <fmt:formatNumber value="${cartTotal }" pattern="#,##0" /> 원
				</td>
			</tr>
		</tfoot>
	</table>
