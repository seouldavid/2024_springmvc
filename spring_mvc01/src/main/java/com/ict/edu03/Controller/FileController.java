...
public class FileController {
	...
	public void FileDown(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			String f_name = request.getParameter("f_name"); // request에서 파일이름을 얻는다.
			System.out.println("f_name: " + f_name);
			String path = request.getSession().getServletContext().getRealPath("/resources/upload/"+f_name);// request에서 그파일의 실제 경로를 얻는다.
		
			String r_path = URLEncoder.encode(f_name, "utf-8"); // 파일이름을 MIME 타입으로 인코딩 한다
			System.out.println("r_path: " + r_path);
			// 브라우저 설정
			response.setContentType("application/x-msdownload"); //컨텐츠 타입을 다운로드 타입으로 바꾼다.
